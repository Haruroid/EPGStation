...
クソースを用いて全録仕様に改造しているため、普通の使い方はできなくなっています。別々で使ってください。

デフォルトでは番組表が1週間なので1週間前の番組を消すようになっています。変えたい場合は```EPGStation/src/model/operator/reservation/ReservationManageModel.ts:997```の変数をいじってください。

プロテクトをかけた番組は消えないはずです。残したい番組等に活用してください。

逆アセンブルで書いたようなクソースなので動作は完全に未保証です。